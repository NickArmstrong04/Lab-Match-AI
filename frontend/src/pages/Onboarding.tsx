...
export const Onboarding: React.FC<OnboardingProps> = ({
  ...
  const handleGoogleLogin = () => {
    ...
    // Open OAuth window popup for login
    window.open(
      `${baseUrl}/auth/google/login?student_id=login`,
      'Google OAuth Login',
      `width=${width},height=${height},left=${left},top=${top},status=no,toolbar=no,menubar=no`
    );
  };
            ...
            <form onSubmit={handleLogin} className="space-y-5 flex-1 flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <div className="relative my-4 flex items-center justify-center">
                  <span className="absolute inset-x-0 h-px bg-stone-200"></span>
                  <span className="relative px-3 bg-white text-[10px] font-bold text-stone-400 font-mono uppercase tracking-wider">OR</span>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-lg border border-stone-300 hover:border-stone-400 text-stone-700 hover:bg-stone-50 font-semibold inline-flex items-center justify-center gap-2.5 transition-all text-xs cursor-pointer bg-white"
                >
                  <svg className="w-4 h-4 block" viewBox="0 0 24 24">
                    <path
                      fill="#4285F4"
